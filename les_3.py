    ...
    try:
        ...
        try:
            if salary_split_list[0] == 'от':
                vacancy_dict['min_salary'] = int(salary_split_list[1])
            elif salary_split_list[0] == 'до':
                vacancy_dict['max_salary'] = int(salary_split_list[1])
            else:
                vacancy_dict['min_salary'] = int(salary_split_list[0])
                vacancy_dict['max_salary'] = int(salary_split_list[2])
        except:
            pass
...
db.vacancies.insert_many(vacancies_list)

# Вывод на экран вакансия с заработной платой больше введенной суммы
for x in db.vacancies.find({ "$or": [ {"min_salary": {"$gt": 50000}}, {"max_salary": {"$gt": 50000}}]}):
    print(x)

# Добавление новый вакансий
for x in vacancies_list:
    if db.vacancies.count_documents({ "url": x['url'] }) == 0:
        db.vacancies.insert_one(x)
